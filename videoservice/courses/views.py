...
class LessonDetailView(View):
 # filter all the courses, and then filter that course's lessons for a specific lesson
 # ..yeah wtf
    def get(self, request, course_slug, lesson_slug, *args, **kwargs):

        # create course qs and filter it by slug.
        course_queryset = Course.objects.filter(slug=course_slug)
        if course_queryset.exists():
            course = course_queryset.first()

        # grabs all lessons by course
        ...
        # grab courses allowed by membership type
